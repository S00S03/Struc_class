...
void List::printConnections() const {
    ...
    do {
        std::cout << "[ " << current->value << " | prev: "
                  << (current->prev ? std::to_string(current->prev->value)
                                    : "NULL")
                  << " | next: "
                  << (current->next ? std::to_string(current->next->value)
                                    : "NULL")
                  << " ]" << std::endl;
        current = current->next;
    } while (current != head);
...
int List::getFront() const { return head->value; }

int List::getBack() const { return head->prev->value; }
